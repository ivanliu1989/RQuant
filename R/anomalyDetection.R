...
#'
#' @examples
#' raw_data <- data.frame(zoo::index(AUDUSD), drop(zoo::coredata((diff(AUDUSD)))))
...
#' @name raw_data
#' @examples
#' raw_data <- drop(zoo::coredata((diff(AUDUSD))))
#' # To detect only the anomalies in the last period, run the following:
#' AnomalyDetectionVec(raw_data, max_anoms=0.02, period=1440, direction='both', only_last=TRUE, plot=TRUE)
...
#'
#' @param data Time series to perform anomaly detection on.
#' @param k Maximum number of anomalies that S-H-ESD will detect as a percentage of the data.
#' @param alpha The level of statistical significance with which to accept or reject anomalies.
#' @param num_obs_per_period Defines the number of observations in a single period, and used during seasonal decomposition.
#' @param use_decomp Use seasonal decomposition during anomaly detection.
#' @param use_esd Uses regular ESD instead of hybrid-ESD. Note hybrid-ESD is more statistically robust.
#' @param one_tail If TRUE only positive or negative going anomalies are detected depending on if upper_tail is TRUE or FALSE.
#' @param upper_tail If TRUE and one_tail is also TRUE, detect only positive going (right-tailed) anomalies. If FALSE and one_tail is TRUE, only detect negative (left-tailed) anomalies.
#' @param verbose Additionally printing for debugging.
#'
#' @return A list containing the anomalies (anoms) and decomposition components (stl).
#'
#' @export
